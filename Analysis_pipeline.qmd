...
kable(
  ...
  digits = 3,
  row.names = FALSE,
  caption = "Table S13c. Pairwise habitat comparisons of coverage-standardised diversity (60% coverage; plants as sampling units). est_1 and est_2 are the estimates for the first and second habitat of each pair, ratio = est_1 / est_2, ci_overlap indicates whether the two 95% bootstrap confidence intervals overlap, and z and p_holm are from a z-test on the bootstrap standard errors, Holm-adjusted across the three pairs."
)
```
...
kable(
  ...
  digits = 4,
  row.names = FALSE,
  caption = "Table S15e. Habitat × site interaction for abundance-weighted PD (q = 1, 2) with and without the logit relative abundance of Helotiales and/or Sebacinales as a covariate. hel_assoc and seb_assoc give the slope and p-value of each lineage covariate on per-plant PD; F and p are for the interaction term under each covariate set."
)
```
...
kable(lin_tests, digits = 4, row.names = FALSE,
      caption = "Table S18b. Likelihood-ratio tests of site-specific trends in Helotiales and Sebacinales relative abundance (beta regression, logit link, n = 60 plants): elevation × site and habitat × site (cell-means parameterisation, which handles the empty habitat × site cells) against the additive model. published_elev_coef and published_elev_p are the elevation coefficient (per m, logit scale) and p-value of the additive model (Tables S16–S17).")
```
...
hel_frac <- hel_frac[, setdiff(names(hel_frac), "note")]
kable(
  hel_frac,
  digits = 4,
  row.names = FALSE,
  caption = "Table S18d. The Helotiales elevation trend split into genus-assigned and genus-unassigned OTUs, and by family. Each fraction is fitted with the same additive beta regression as Table S16 (proportion of total reads per plant ~ elevation + site). share_of_Helotiales_reads = fraction of all Helotiales reads; mean_prop = mean proportion of total reads per plant; odds_change_pct_per_100m = percentage change in the odds per 100 m, 100 × (exp(β) − 1) with β per 100 m on the logit scale; p_elevation_x_site = likelihood-ratio test of site-specific slopes; slope_* = site-specific slopes from the elevation × site model (change in proportion per 100 m)."
)
```
...
kable(
  ...
  digits = 2,
  row.names = FALSE,
  caption = "Table S21. Model structure for per-plant Hill diversity (rarefied to 28,122 reads) under three error families: AICc of the site-only, additive (habitat + site) and interaction (habitat × site) models; dAICc_habitat = AICc(habitat + site) − AICc(site only); dAICc_interaction = AICc(habitat × site) − AICc(habitat + site); best = lowest-AICc model. Log-normal AICc includes the Jacobian of the log transform so that it is comparable with the other families. The reported models are gamma with a log link (gamma_log)."
)
```
...
kable(
  ...
  digits = 4,
  row.names = FALSE,
  caption = "Table S23. Support for habitat in the per-genus negative binomial GLMMs (56 genera; Section 6.3): dAICc_habitat = AICc(habitat + site) − AICc(site only), with habitat_supported_AICc = TRUE where dAICc_habitat < −2; omnibus_p/q = likelihood-ratio test of habitat and its Benjamini–Hochberg q-value across genera; subparamo/paramo_vs_forest_q = BH q-values of the contrasts against forest (NA where the genus is absent from forest or the contrast is not estimable)."
)
```
